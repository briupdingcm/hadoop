...
public class Cooccurence extends Configured implements Tool {
    ...
    static class CoMapper extends Mapper<LongWritable, Text, Text, IntWritable> {
    ...
    static class CoReducer extends Reducer<Text, IntWritable, Text, IntWritable> {
...
//			context.write(key, counter);
            //Collectors
            //		int count = StreamSupport.stream(values.spliterator(), true).collect(summingInt(x -> x.get()));
            int count = StreamSupport.stream(values.spliterator(), true).mapToInt(x -> x.get()).reduce(
                    0,
                    (sum, e) -> sum + e);
            counter.set(count);
            context.write(key, counter);
